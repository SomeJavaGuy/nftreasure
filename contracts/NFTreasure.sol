...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
        ...
        virtual
    {
        ...
        _beforeTokenTransfer(operator, from, to, _asSingletonArray(id), _asSingletonArray(1), "");

        _balances[id][from] = _balances[id][from].sub(1, "ERC1155: insufficient balance for transfer");
        _balances[id][to] = _balances[id][to].add(1);

        emit TransferSingle(operator, from, to, id, 1);

        _doSafeTransferAcceptanceCheck(operator, from, to, id, 1, "");
    }
...
// File: contracts/NFTreasure.sol

// Milion dollar gallery
// 100 cells. You can buy each cell for N ETH.
...
contract NFTreasure is ERC1155, Ownable {
    ...
    function submitSecret(bytes memory _secret) public {
        ...
        // Add secret to blacklist so that it can't be submitted again
        blacklist[sha3_secret] = true;
    }

    /* Override the default tokenURI function of ERC1155. We want to return the
     * right URL for most tokens, but if the token is a secret one, we want to
